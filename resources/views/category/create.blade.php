...
<div class="max-w-xl mx-auto mt-10 bg-white p-8 border border-gray-300 rounded-lg shadow-lg">
    <h2 class="text-2xl font-bold mb-6 text-gray-800">Agregar Categoria</h2>
    <form action="{{ route('category.store') }}" method="POST" class="space-4">
        @csrf
        @if(session('success'))
        ...
        <div>
            ...
            <input type="text" id="name" name="name" class="mt-2 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Ej. Nombre de la categoria">
            @error('name')
        ...
        <div class="mt-2">
            ...
            <textarea name="description" id="description" rows="6" class="mt-2 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Escribe aquí la descripción de la categoria"></textarea>
        </div>
